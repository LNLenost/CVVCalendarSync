    ...
    try:
        ...
        for event in agenda["agenda"]:
            ...
            for existing_event in existing_events:
                if existing_event['summary'] == event["notes"]:
                    existing_event_start = existing_event['start'].get(
                        'dateTime',
                        existing_event['start'].get('date')
                    )
                    existing_event_end = existing_event['end'].get(
                        'dateTime',
                        existing_event['end'].get('date')
                    )
                    if existing_event_start[:10] == event["evtDatetimeBegin"][:10]:
                        print(f"Rimuovo vecchio evento: {existing_event['summary']} - "
                              f"{existing_event_start} to {existing_event_end}")
                        service.events().delete(
                            calendarId=calendar_id,
                            eventId=existing_event['id']
                        ).execute()
                        
            print(f"Aggiungo evento: {event['notes']} - "
                  f"{event['evtDatetimeBegin']} to {event['evtDatetimeEnd']}")
            event_body = {
                "summary": event["notes"],
                "location": "",
        ...
        for existing_event in existing_events:
            ...
            if not event_in_agenda:
                ...
                print(f"Rimuovo evento eliminato da ClasseViva: "
                      f"{existing_event['summary']} - {start_time} to {end_time}")
                service.events().delete(
                    calendarId=calendar_id,
                    eventId=existing_event['id']
                ).execute()
        ...
        print(f"Si è verificato un errore nel sincronizzare "
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            if (error_json.get("statusCode") == 422 and
                    "school year not started yet" in error_json.get("error", "")):
                print("\n[INFO] L'anno scolastico non è ancora iniziato. "
                      "Riprova quando sarà attivo nel registro elettronico.\n")
                return None
        except Exception:
            pass
...
def cli():
    ...
    if not os.path.exists(config_file):
        ...
        print("\n🔧 SOLUZIONE:")
        print("1. Assicurati di aver seguito le istruzioni di configurazione")
    ...
    if not all([user_id, user_pass, calendar_id, credentials_file]):
        print("❌ ERRORE: Configurazione incompleta in config.json!")
        print("🔧 SOLUZIONE:")
        print("   Assicurati che config.json contenga tutti i campi richiesti:")
        print("   - user_id")
        print("   - user_pass")
        print("   - calendar_id")
    ...
    if not os.path.exists(credentials_file):
        ...
        print("\n🔧 SOLUZIONE:")
        print("1. Scarica il file credentials.json da Google Cloud Console")
